...
// Saida: vet
{
  ...
  // INICIALIZANDO DINAMICAMENTE
  vet->v = (double *) malloc(tam * sizeof(double));

  // verifica se alocacao foi correta
  erroAssert(vet->v != NULL, "Erro ao alocar vetor!");
...
// Saida: dst
{
  ...
  // se o vetor C nao existir antes...
  if (dst->v == NULL)
  {
    criaVetor(dst,src->tam, dst_id);
    inicializaVetorNulo(dst);
  }else{ // se o vetor C ja existe
    inicializaVetorNulo(dst);
  }
